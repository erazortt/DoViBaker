...
- `_dovi_dynamic_max_pq` the max_pq of the current scene read from the stream
...
This plugin uses the frame property `_dovi_dynamic_max_content_light_level` set by either `DoViBaker` or `DoViSceneFileReader` to apply different LUTs based on thresholds.

## Integrated LUT processing
This is done by providing a collection of LUTs and limits of validity measured in nits of max-content-light-level. These will then be processed internally. (The LUT processing implentation is based on: https://github.com/sekrit-twc/timecube).
```
DoViBaker(bl,el)
...
This plugin reads the two scene files generated by the avisynth script `DoViSceneFileCreator.avs`. It can be used for sources which do not have any DolbyVision substream, but where a the LUT processing by DoViCubes is still necessary.

```
DoViSceneFileReader("sceneCut.txt","maxPq.txt")
DoViCubes(cubes="lut_1000.cube;lut_2000.cube;lut_4000.cube",mclls="1010;2020",cubes_basepath="C:\")
```
