...
This plugin uses the frame property `_dovi_dynamic_max_content_light_level` set by either `DoViBaker` or `DoViMaxPqFileReader` to apply different LUTs based on thresholds.
...
This plugin reads the two scene files generated by the avisynth script `DoViMaxPqFileCreator.avs`. It can be used for sources which do not have any DolbyVision substream, but where a the LUT processing by DoViCubes is still necessary.

```
DoViMaxPqFileReader("sceneCut.txt","maxPq.txt")
DoViCubes(cubes="lut_1000.cube;lut_2000.cube;lut_4000.cube",mclls="1010;2020",cubes_basepath="C:\")
```
