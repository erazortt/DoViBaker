...
# DoViCubes
This plugin provides LUT processing capabilites based on the frame property `_dovi_dynamic_max_content_light_level` set by either `DoViBaker` or `DoViMaxPqFileReader`. Different LUTs are applied based adjustable thresholds. This is done by providing a collection of LUTs and limits of validity measured in nits of max-content-light-level. (The LUT processing implentation is based on: https://github.com/sekrit-twc/timecube).
```
DoViBaker(bl,el)
...
# DoViMaxPqFileReader
This plugin reads the maxPq file generated by the avisynth script `DoViMaxPqFileCreator.avs`. It can be used for sources which do not have any DolbyVision substream, but where a the LUT processing by DoViCubes is still necessary. Additionally it is possible to provide an alternative scene cut file, created by other means.

```
DoViMaxPqFileReader("maxPq.txt")
DoViCubes(cubes="lut_1000.cube;lut_2000.cube;lut_4000.cube",mclls="1010;2020",cubes_basepath="C:\")
```
