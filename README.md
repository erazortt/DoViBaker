...
This plugin reads the maxPq file generated by the avisynth script `DoViMaxPqFileCreator.avs`. It can be used for sources which do not have any DolbyVision substream, but where a the LUT processing by DoViCubes is still necessary. Additionally it is possible to provide an optional separate scene cut file.
