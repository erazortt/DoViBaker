...
This plugin uses the frame property `_dovi_max_content_light_level` set by either `DoViBaker` or `DoViSceneFileReader` to apply different LUTs based on thresholds.
...
This plugin reads the scene file generated by the avisynth script `DoViSceneFileCreator.avs`. It can be used for sources which do not have any DolbyVision substream, but where a the LUT processing by DoViCubes is still necessary.

```
DoViSceneFileReader("dovi.scene.file.txt")
DoViCubes(cubes="lut_1000.cube;lut_2000.cube;lut_4000.cube",mclls="1010;2020",cubes_basepath="C:\")
```
