...
In that case, the generated LUT will expect that the input PQ was re-normalized to 1000 nits max brightness, meaning that [DoViTonemap](#dovitonemap) was used with the option `normalizeOutput` set to `true`. LUTs for re-normalized inputs can be of smaller size than normal LUTs while still providing better quality. A good size for such a LUT is `50` instead of the `65` needed for the default PQ input.
